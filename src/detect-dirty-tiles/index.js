...
function queryChanges(task, timestamp, viewName) {
    return task.query(
...
function affectedTiles(task, timestamp, viewName, minZoomLevel, maxZoomLevel) {
    return queryChanges(task, timestamp, viewName).then(function(rows) {
        return rows.map(function(row) {
            return {
                view: viewName,
                osmId: row.osmId,
        ...
        return t.one('SELECT MAX(timestamp) FROM osm_timestamps')
            .then(function (result) {
                ...
                function detectDirtyTiles(viewName, minZoomLevel, maxZoomLevel) {
                    return affectedTiles(t, timestamp, viewName, minZoomLevel, maxZoomLevel);
                }
